...
describe('productUtils', () => {
    ...
    describe('postProduct', () => {
        const expectedQuery = `INSERT INTO products (nume, cantitate, tip, data_expirare, adresa, luna, an, user_id) value ('${mockReq.body.nume}', '${mockReq.body.cantitate}', '${mockReq.body.tip}', '${mockReq.body.data_expirare}', '${mockReq.body.adresa}', '${mockCurrentMonth + 1}', '${mockCurrentYear}', '${mockReq.body.user_id}')`;
    ...
    describe('putProduct', () => {
        ...
        test('it modifies a product', () => {
            productUtils.putProduct(mockReq, mockRes);

            expect(con.query).toHaveBeenCalledWith(expectedQuery, expect.any(Function));
        });

        test('it calls res with modify "false" if it errors', () => {
            const errorStatus = true;
            con.query.mockImplementationOnce((_, fn) => {
                fn(errorStatus);
            });
        ...
        test('it calls res with modify "true" if it is successful', () => {
            const errorStatus = false;
            con.query.mockImplementationOnce((_, fn) => {
                fn(errorStatus);
            });
    ...
    describe('deleteProduct', () => {
        ...
        test('it modifies a product', () => {
            productUtils.deleteProduct(mockReq, mockRes);

            expect(con.query).toHaveBeenCalledWith(expectedQuery, expect.any(Function));
        });

        test('it calls res with delete "false" if it errors', () => {
            const errorStatus = true;
            con.query.mockImplementationOnce((_, fn) => {
                fn(errorStatus);
            });
        ...
        test('it calls res with delete "true" if it is successful', () => {
            const errorStatus = false;
            con.query.mockImplementationOnce((_, fn) => {
                fn(errorStatus);
            });
